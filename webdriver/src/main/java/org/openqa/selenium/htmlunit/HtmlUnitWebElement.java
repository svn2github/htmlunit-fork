...
public class HtmlUnitWebElement implements WebElement, FindsById,
    ...
    private final static String[] blockLevelsTagNames = { "p", "h1", "h2",
            "h3", "h4", "h5", "h6", "dl", "div", "noscript", "blockquote",
            "form", "hr", "table", "fieldset", "address", "ul", "ol", "pre",
            "br" };
        ...
        try {
            ...
            if (form == null)
                throw new NoSuchElementException(
                        "Unable to find the containing form");
            form.submit();
        } catch (IOException e) {
            throw new WebDriverException(e);
        }
    ...
    private void submitForm(HtmlForm form) {
        ...
        names.add("input");
        names.add("button");
        List<? extends HtmlElement> allElements = form
                .getHtmlElementsByTagNames(names);
    ...
    private boolean isSubmitElement(HtmlElement element) {
        ...
        if (element instanceof HtmlSubmitInput
                && !((HtmlSubmitInput) element).isDisabled())
            candidate = element;
        else if (element instanceof HtmlImageInput
                && !((HtmlImageInput) element).isDisabled())
            candidate = element;
        else if (element instanceof HtmlButton) {
            HtmlButton button = (HtmlButton) element;
            if ("submit".equalsIgnoreCase(button.getTypeAttribute())
                    && !button.isDisabled())
                candidate = element;
        }
    ...
    public void sendKeys(CharSequence... value) {
        ...
        if (element instanceof HtmlInput) {
            ...
            element.setAttribute("value", (currentValue == null ? ""
                    : currentValue)
                    + builder.toString());
        } else if (element instanceof HtmlTextArea) {
            String currentValue = getValue();
            ((HtmlTextArea) element).setText((currentValue == null ? ""
                    : currentValue)
                    + builder.toString());
        } else {
    ...
    public String getAttribute(String name) {
        ...
        if ("readonly".equalsIgnoreCase(lowerName)) {
            ...
            if (element instanceof HtmlTextArea) {
                return "".equals(((HtmlTextArea) element)
                        .getReadOnlyAttribute()) ? "false" : "true";
            }
    ...
    public boolean isSelected() {
        ...
        throw new UnsupportedOperationException(
                "Unable to determine if element is selected. Tag name is: "
                        + element.getTagName());
    }
    ...
    public String getText() {
        ...
        getTextFromNode(element, toReturn, textSoFar,
                element instanceof HtmlPreformattedText);
    ...
    private void getTextFromNode(DomNode node, StringBuffer toReturn,
            StringBuffer textSoFar, boolean isPreformatted) {
    ...
    private String collapseWhitespace(StringBuffer textSoFar) {
        ...
        return textToAdd.replaceAll("\\p{javaWhitespace}+", " ").replaceAll(
                "\r", "");
    }
    ...
    private void getPreformattedText(DomNode node, StringBuffer toReturn) {
        if (node.isDisplayed()) {
            ...
            toReturn.append(xmlText.replaceAll("^<pre.*?>", "").replaceAll(
                    "</pre.*>$", ""));
        }
    }
    ...
    public WebElement findElementByXPath(String xpathExpr) {
        ...
        if (match == null) {
            throw new NoSuchElementException(
                    "Unable to find element with xpath " + xpathExpr);
        }
    ...
    public WebElement findElementByTagName(String name) {
        ...
        if (elements.size() == 0) {
            throw new NoSuchElementException(
                    "Cannot find element with tag name: " + name);
        }
    ...
    public List<WebElement> findElementsByTagName(String name) {
        ...
        // // TODO(simon.m.stewart): Update this once the next version of
        // HtmlUnit is released
        // NodeList elements = element.getElementsByTagName(name);
        // ArrayList<WebElement> toReturn = new
        // ArrayList<WebElement>(elements.getLength());
        // for (int i = 0; i < elements.getLength(); i++) {
        // toReturn.add(parent.newHtmlUnitWebElement((HtmlElement)
        // elements.item(i)));
        // }
        //
        // return toReturn;
